# Individual Contributor Assignment Agreement (ICAA)

> **DRAFT — REQUIRES QUALIFIED SOFTWARE/IP LEGAL REVIEW BEFORE USE**

This Individual Contributor Assignment Agreement ("Agreement") is entered into by and between the undersigned individual contributor ("Contributor") and `[COPYRIGHT OWNER NAME / DESIGNATED ENTITY]` ("Project Owner") regarding contributions made to the SLAVES project (`https://github.com/Zen1th53/slaves`).

---

## 1. Definitions

* **"Contribution"**: Any source code, documentation, bug fix, patch, test case, schema, design, or other material intentionally submitted by Contributor for inclusion in, or documentation of, the SLAVES project.
* **"Submitted"**: Any form of electronic, verbal, or written communication sent to Project Owner or its representatives, including but not limited to communication on electronic mailing lists, source code control systems, and issue tracking systems.

---

## 2. Copyright Assignment

1. **Assignment**: Subject to accepted legal review and acceptance by Project Owner, Contributor hereby assigns, transfers, and conveys to Project Owner all worldwide right, title, and interest, including all copyrights and copyright renewal rights, in and to all Contributions accepted for incorporation into the SLAVES project.
2. **Authorship & History Preservation**: Contributor retains moral recognition of authorship. Project Owner agrees that Contributor's authorship credit will be maintained in Git commit history and public version control records. Authorship credit does not modify Project Owner's sole copyright ownership of accepted Contributions.

---

## 3. Scope of Project Owner Licensing Rights & Future Transfers

By virtue of the copyright assignment in Section 2, Project Owner (and its legal successors and assigns) possesses full, unrestricted authority over accepted Contributions, including without limitation the right to:

* Distribute Contributions under the GNU Affero General Public License v3.0 only (`AGPL-3.0-only`);
* Distribute Contributions under proprietary commercial license agreements;
* Dual-license, sublicense, relicense, or modify licensing models for future project versions;
* Enforce copyrights against unauthorized third-party infringement; and
* **Assign, sell, transfer, or convey** all assigned copyrights and project IP assets to a successor entity, parent company, or acquiring entity in connection with a merger, acquisition, corporate reorganization, or sale of assets **without requiring additional consent or approval from Contributor**.

---

## 4. Patent License & Grant

* **Patent Grant**: Contributor hereby grants to Project Owner, and to recipients of software distributed by Project Owner, a perpetual, worldwide, non-exclusive, no-charge, royalty-free, irrevocable patent license to make, have made, use, offer to sell, sell, import, and otherwise transfer the Contribution.
* **Scope**: This patent grant applies strictly to those patent claims licensable by Contributor which are necessarily infringed by Contributor's Contribution alone or by combination of the Contribution with the SLAVES project to which such Contribution was submitted.

> [!IMPORTANT]
> *Attorney Review Flag*: The exact scope of patent grant language must be reviewed by qualified IP counsel prior to operational execution.

---

## 5. Contributor Representations & Warranties

Contributor represents and warrants that:

1. Contributor is the sole author and has full legal authority to grant the copyright assignment and patent rights set forth herein;
2. The Contribution is Contributor's original creation and does not knowingly infringe any third-party copyright, trademark, patent, trade secret, or proprietary right;
3. If Contributor is employed or under contract, Contributor either (a) owns all IP rights in the Contribution personally, or (b) has obtained explicit written permission or waiver from their employer/client authorizing this assignment; and
4. Any third-party code, open-source dependency, or AI-generated material included within or used to produce the Contribution has been explicitly disclosed in writing prior to submission.

---

## 6. Moral Rights Waiver

To the maximum extent permitted by applicable law in any jurisdiction, Contributor waives, or covenants not to assert against Project Owner, its successors, assigns, licensees, and downstream users, any "moral rights" (including rights of integrity and attribution beyond standard Git commit history) in accepted Contributions.

> [!IMPORTANT]
> *Attorney Review Flag*: Moral rights waiver enforceability varies by jurisdiction and must be vetted by local IP counsel.

---

## 7. Signature & Execution

```text
Contributor Name:      ____________________________________
GitHub Username:       ____________________________________
Email Address:         ____________________________________
Date:                  ____________________________________
Signature:             ____________________________________

Accepted by Project Owner:
Project Owner / Rep:   [COPYRIGHT OWNER NAME / DESIGNATED ENTITY]
Date:                  ____________________________________
Signature:             ____________________________________
```
