...
```text
             ...
             ┌─────────────────────┴─────────────────────┐
             │                                           │
      Community Edition                     Commercial License
       AGPL-3.0-only                        Separate Agreement
             │                                           │
    • Open-source use                          • Proprietary integration
    • Network copyleft                         • Relieves AGPL obligation
    • Source disclosure                        • Custom/OEM/Enterprise
```
...
> [!NOTE]
> Commercial licensing does **not** imply that all corporate use requires payment. If an organization fully complies with the terms of the `AGPL-3.0-only`, it is free to use the Community Edition.

---

## 3. Inquiry & Procurement
...
Previous public releases up to `runtime-v0.4.0` were distributed under the Apache License 2.0 (`Apache-2.0`). Historical grants associated with those previously distributed versions remain valid and associated with those versions. See [`docs/legal/LICENSE-HISTORY.md`](docs/legal/LICENSE-HISTORY.md) for complete details.
