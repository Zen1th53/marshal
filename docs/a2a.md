...
### Start Server
```bash
marshal a2a serve --listen 127.0.0.1:8081
```
...
```bash
curl -s -H "Authorization: Bearer $MARSHAL_TOKEN" \
  http://127.0.0.1:8081/.well-known/agent-card.json
```

### Message Send (`POST /message:send`)
Dispatches structured A2A task delegation messages to the MARSHAL control plane.
...
curl -s -X POST http://127.0.0.1:8081/message:send \
  ...
  -d '{
    ...
    "recipient": { "id": "MARSHALControlPlane", "role": "developer" },
    "parts": [
      {
...
MARSHAL enforces strict role boundaries on A2A delegation:
