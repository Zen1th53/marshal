...
This patch release hardens the first stable MARSHAL release after clean-user
and local-provider verification. It fixes long-running daemon responses, empty
provider-output persistence, and immutable base-commit binding while replacing
the initial release pipeline with the complete evidence-backed gate.
...
- Build-aware version reporting, deterministic Linux archives, CycloneDX SBOM,
  checksums, build metadata, and GitHub provenance
...
```bash
curl -LO https://github.com/Zen1th53/marshal/releases/download/v1.0.1/marshal_1.0.1_linux_amd64.tar.gz
curl -LO https://github.com/Zen1th53/marshal/releases/download/v1.0.1/checksums.txt
...
See [Getting started](https://github.com/Zen1th53/marshal/blob/v1.0.1/docs/getting-started.md)
...
```bash
gh attestation verify marshal_1.0.1_linux_amd64.tar.gz --repo Zen1th53/marshal
```
...
protected action. Read [SECURITY.md](https://github.com/Zen1th53/marshal/blob/v1.0.1/SECURITY.md).
...
- Release assets are deterministically generated in the release gate, but the
  project does not claim independent cross-environment reproducible builds.
...
[full changelog](https://github.com/Zen1th53/marshal/blob/v1.0.1/CHANGELOG.md)
and [licensing terms](https://github.com/Zen1th53/marshal/blob/v1.0.1/LICENSING.md).
