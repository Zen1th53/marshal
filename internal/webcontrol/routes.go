...
func (s *Server) registerRoutes(mux *http.ServeMux) {
	...
	// 3. Agents (read = authenticated; mutations = policy.admin)
	mux.HandleFunc("GET /api/v1/agents", s.RequireAuth(s.handleListAgents))
	mux.HandleFunc("POST /api/v1/agents", s.RequireAuthority(authPolicyAdmin, s.handleCreateAgent))
	mux.HandleFunc("GET /api/v1/agents/{id}", s.RequireAuth(s.handleGetAgentDetail))
	mux.HandleFunc("PATCH /api/v1/agents/{id}", s.RequireAuthority(authPolicyAdmin, s.handleUpdateAgent))
	mux.HandleFunc("DELETE /api/v1/agents/{id}", s.RequireAuthority(authPolicyAdmin, s.handleDeleteAgent))
	...
	mux.HandleFunc("PATCH /api/v1/providers/{id}", s.RequireAuthority(authPolicyAdmin, s.handleUpdateProvider))
	mux.HandleFunc("POST /api/v1/providers/{id}/secret", s.RequireAuthority(authPolicyAdmin, s.handleSetProviderSecret))
	mux.HandleFunc("POST /api/v1/providers/{id}/probe", s.RequireAuth(s.handleProbeProvider))
	...
	mux.HandleFunc("POST /api/v1/security/policy/draft", s.RequireAuthority(authPolicyAdmin, s.handleSavePolicyDraft))
	mux.HandleFunc("POST /api/v1/security/policy/validate", s.RequireAuthority(authPolicyAdmin, s.handleValidatePolicy))
	mux.HandleFunc("POST /api/v1/security/policy/diff", s.RequireAuthority(authPolicyAdmin, s.handleDiffPolicy))
	mux.HandleFunc("POST /api/v1/security/policy/apply", s.RequireAuthority(authPolicyAdmin, s.handleApplyPolicy))
	mux.HandleFunc("POST /api/v1/security/policy/rollback", s.RequireAuthority(authPolicyAdmin, s.handleRollbackPolicy))
