...
// PutCapabilityGrant stores a validated grant as an immutable record. Repeating
// the exact grant is idempotent; changing an existing grant is a conflict.
func (s *Store) PutCapabilityGrant(ctx context.Context, grant capability.Grant) error {
	...
	`, string(grant.ID), string(grant.Subject), string(grant.TaskID), string(grant.Kind), grant.Scope.Resource,
		string(actions), string(constraints), string(grant.Issuer), issuedAt, expiresAt, formatOptionalTime(grant.RevokedAt), grant.PolicyDigest, grant.IdempotencyKey)
	if err == nil {
		return nil
	}
...
func (s *Store) GetCapabilityGrant(ctx context.Context, id capability.GrantID) (capability.Grant, error) {
	...
	grant.Subject = capability.SubjectID(subject)
	grant.TaskID = capability.TaskID(taskID)
	grant.Kind = capability.CapabilityKind(kind)
...
// RevokeCapabilityGrant is a compare-and-set transition. A second revoke is
// rejected so callers cannot mistake an already-revoked record for a new write.
func (s *Store) RevokeCapabilityGrant(ctx context.Context, id capability.GrantID, revokedAt time.Time) error {
	...
	`, revokedAt.UTC().Format(time.RFC3339Nano), string(id))
	if err != nil {
...
func capabilityGrantsEqual(left, right capability.Grant) bool {
	...
	return left.ID == right.ID && left.Subject == right.Subject && left.TaskID == right.TaskID &&
		left.Kind == right.Kind && left.Scope.Resource == right.Scope.Resource &&
		string(leftActions) == string(rightActions) &&
		string(leftConstraints) == string(rightConstraints) && left.Issuer == right.Issuer &&
		left.IssuedAt.UTC().Equal(right.IssuedAt.UTC()) && left.ExpiresAt.UTC().Equal(right.ExpiresAt.UTC()) &&
		optionalTimesEqual(left.RevokedAt, right.RevokedAt) && left.PolicyDigest == right.PolicyDigest &&
