...
func TestCapabilityGrantPersistenceRoundTripAndIdempotency(t *testing.T) {
	...
	grant := capability.Grant{
		...
		Scope:    capability.Scope{Resource: "/workspace/task-1", Actions: []string{"write"}, Constraints: map[string]string{"worktree": "/workspace/task-1"}},
	...
	if loaded.ID != grant.ID || loaded.Subject != grant.Subject || loaded.TaskID != grant.TaskID || loaded.Kind != grant.Kind || loaded.Scope.String() != grant.Scope.String() || loaded.PolicyDigest != grant.PolicyDigest || !loaded.ExpiresAt.Equal(grant.ExpiresAt) {
...
func TestCapabilityGrantPersistenceReopensAndRevokesWithCAS(t *testing.T) {
	...
	now := time.Date(2026, 8, 17, 13, 0, 0, 0, time.UTC)
	grant := capability.Grant{ID: "CAP-A02-2", Subject: "agent-2", TaskID: "task-2", Kind: capability.KindGitPush, Scope: capability.Scope{Resource: "repo-2", Actions: []string{"push"}}, IssuedAt: now, ExpiresAt: now.Add(time.Hour), Issuer: "broker"}
...
func TestCapabilityGrantRejectsInvalidDigestAndPreIssueRevoke(t *testing.T) {
	...
	now := time.Date(2026, 8, 17, 13, 0, 0, 0, time.UTC)
	grant := capability.Grant{ID: "CAP-A02-3", Subject: "agent-3", TaskID: "task-3", Kind: capability.KindSecretUse, Scope: capability.Scope{Resource: "secret://task-3", Actions: []string{"use"}}, IssuedAt: now, ExpiresAt: now.Add(time.Hour), Issuer: "broker", PolicyDigest: "MARSHAL_TEST_SECRET_T01_A02_invalid_digest"}
