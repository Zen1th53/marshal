...
func (s *Store) Migrate(ctx context.Context) error {
	...
	if version == 27 {
		// `handoffs` is legacy free-form compatibility history. Typed handoffs
		// remain separate and are the only canonical T28 state.
		if _, err := tx.ExecContext(ctx, `
		...
		`); err != nil {
			return fmt.Errorf("apply schema version 28: %w", err)
		}
