...
func (s *Store) Migrate(ctx context.Context) error {
	...
	if version < 61 {
		if _, err := tx.ExecContext(ctx, `
			CREATE TABLE doctor2_check_runs (
				run_id TEXT PRIMARY KEY,
				check_id TEXT NOT NULL,
				status TEXT NOT NULL DEFAULT 'PASSED',
				created_at TEXT NOT NULL
			);
			CREATE INDEX doctor2_check_runs_by_check
				ON doctor2_check_runs(check_id, created_at);
		`); err != nil {
			return fmt.Errorf("migrate schema version 61: %w", err)
		}
		if _, err := tx.ExecContext(ctx, "INSERT INTO schema_migrations(version, applied_at) VALUES(61, ?)", utcNow()); err != nil {
	...
	if version < 63 {
		if _, err := tx.ExecContext(ctx, `
			...
			CREATE INDEX evidence_trust_scores_by_digest
				ON evidence_trust_scores(change_digest, created_at);
		`); err != nil {
			return fmt.Errorf("migrate schema version 63: %w", err)
		}
		if _, err := tx.ExecContext(ctx, "INSERT INTO schema_migrations(version, applied_at) VALUES(63, ?)", utcNow()); err != nil {
	...
	if version < 66 {
		if _, err := tx.ExecContext(ctx, `
			CREATE TABLE evolution_lab_runs (
				run_id TEXT PRIMARY KEY,
				best_individual_id TEXT NOT NULL,
				status TEXT NOT NULL DEFAULT 'COMPLETED',
				created_at TEXT NOT NULL
			);
			CREATE INDEX evolution_lab_runs_by_individual
				ON evolution_lab_runs(best_individual_id, created_at);
		`); err != nil {
			return fmt.Errorf("migrate schema version 66: %w", err)
		}
		if _, err := tx.ExecContext(ctx, "INSERT INTO schema_migrations(version, applied_at) VALUES(66, ?)", utcNow()); err != nil {
			return fmt.Errorf("record schema version 66: %w", err)
		}
		version = 66
	}
	if version < 67 {
		if _, err := tx.ExecContext(ctx, `
			...
			CREATE INDEX merge_queue_entries_by_change
				ON merge_queue_entries(change_id, created_at);
		`); err != nil {
			return fmt.Errorf("migrate schema version 67: %w", err)
		}
		if _, err := tx.ExecContext(ctx, "INSERT INTO schema_migrations(version, applied_at) VALUES(67, ?)", utcNow()); err != nil {
			return fmt.Errorf("record schema version 67: %w", err)
		}
		version = 67
	}
	if version < 68 {
	...
	if version < 69 {
		if _, err := tx.ExecContext(ctx, `
			...
			CREATE INDEX IF NOT EXISTS memory_outbox_by_unprocessed
				ON memory_outbox(project_id, processed_at, created_at);
		`); err != nil {
			return fmt.Errorf("migrate schema version 69: %w", err)
		}
		if _, err := tx.ExecContext(ctx, "INSERT INTO schema_migrations(version, applied_at) VALUES(69, ?)", utcNow()); err != nil {
			return fmt.Errorf("record schema version 69: %w", err)
		}
		version = 69
	}
	if version < 70 {
	...
	if version < 73 {
		if _, err := tx.ExecContext(ctx, `
			CREATE TABLE IF NOT EXISTS goal_contracts (
				...
				critical_claims_json      TEXT NOT NULL DEFAULT '[]',
				understanding_state       TEXT NOT NULL CHECK(understanding_state IN ('READY','NEEDS_INPUT')),
				unresolved_decisions_json TEXT NOT NULL DEFAULT '[]',
			...
			CREATE INDEX IF NOT EXISTS idx_goal_contracts_session ON goal_contracts(session_id, revision);

			CREATE TABLE IF NOT EXISTS goal_active (
				session_id      TEXT PRIMARY KEY,
				active_goal_id  TEXT NOT NULL,
				active_revision INTEGER NOT NULL,
				updated_at      TEXT NOT NULL,
				FOREIGN KEY(active_goal_id, active_revision) REFERENCES goal_contracts(goal_id, revision)
			);
		`); err != nil {
			return fmt.Errorf("migrate schema version 73: %w", err)
		}
		if _, err := tx.ExecContext(ctx, "INSERT INTO schema_migrations(version, applied_at) VALUES(73, ?)", utcNow()); err != nil {
