...
func (s *Store) appendTypedHandoffEvent(ctx context.Context, handoff protocol.Handoff, status protocol.Status) error {
	eventType := events.EventTypeHandoffCreated
	suffix := "CREATED"
	at := handoff.CreatedAt.UTC()
	if status == protocol.StatusAccepted {
		eventType, suffix = events.EventTypeHandoffAccepted, "ACCEPTED"
	} else if status == protocol.StatusRejected {
		eventType, suffix = events.EventTypeHandoffRejected, "REJECTED"
	} else if status == protocol.StatusConsumed {
		eventType, suffix = events.EventTypeHandoffConsumed, "CONSUMED"
		if handoff.ConsumedAt != nil {
			at = handoff.ConsumedAt.UTC()
		}
	}
	_, err := s.Append(ctx, events.Event{
		ID:             "EVENT-HANDOFF-" + suffix + "-" + string(handoff.ID),
		Type:           eventType,
		Subject:        string(handoff.FromAgent),
		TaskID:         string(handoff.TaskID),
		ResourceID:     string(handoff.ID),
		At:             at,
		IdempotencyKey: "HANDOFF-" + suffix + "-" + string(handoff.ID),
		Data: map[string]any{
